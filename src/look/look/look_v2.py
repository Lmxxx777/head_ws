...
class LookNode(Node):
    def __init__(self):
        super().__init__('look')   
        self.path = os.path.join(os.getcwd(), r'/model/shape_predictor_68_face_landmarks.dat')
        self.face_db = face_db(self.path)
    ...
    def copy_action_callback(self, msg):
        if msg.data == 1:
            # 开启模仿，先初始化，然后打开回调函数
            self.publish_cnt = 1
            self.publish_robot_state([90, 90, 65, 90, 90], self.publish_cnt)
            # 识别关键点，作出反应，发送消息
    ...
    def face_recognation_function(self):
        while(True):
            ...
            if self.last is not None:
                ...
                percentage_eyebrow_left = ['percentage_eyebrow'] - 0.2
                percentage_eyebrow_right = ['percentage_eyebrow'] + 0.4
