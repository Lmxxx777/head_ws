...
rclcpp::Publisher<chat_msgs::msg::State>::SharedPtr speak_pos_pub;  // 说话状态
...
rclcpp::Publisher<std_msgs::msg::Int8>::SharedPtr	rena_awake_pub;
...
auto speak_pos_msg = chat_msgs::msg::State();
...
void speakendCallback(const chat_msgs::msg::State::SharedPtr msg)
{
	if(msg->speak_flag == 0) {
...
int main(int argc, char *argv[])
{
	...
	rclcpp::Subscription<chat_msgs::msg::State>::SharedPtr speakend_sub;
	...
	rena_awake_pub = node->create_publisher<std_msgs::msg::Int8>("awake", 10);

	mic_angle_pub = node->create_publisher<std_msgs::msg::UInt16>("voice_angle", 10);
	speak_pos_pub = node->create_publisher<chat_msgs::msg::State>("speak_pos", 10);
	// gtts 播报请求
	speakend_sub = node->create_subscription<chat_msgs::msg::State>("speak_state", 1000, speakendCallback);
	...
	// 需要用于异常情况播报，已经语音合成的东西
	gttsflag_sub = node->create_subscription<std_msgs::msg::String>("gTTS", 1000, gttsflagCallback);
	...
	printf("--------------case0x02\n");
		if (businessMsg.msgId == 0x01)
		{
			...
			if (major_mic_id <= 5 && major_mic_id >= 0 && mic_angle <= 360 && mic_angle >= 0)
			{
				...
				gTTS("嗯,我在,请提出指令。");
				auto wozai_msg = std_msgs::msg::Int8();
				wozai_msg.data = 1;
				rena_awake_pub->publish(wozai_msg);
